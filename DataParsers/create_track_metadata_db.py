...
def create_db(filename):
    ...
    q += 'track_7digitalid int, shs_perf int, shs_work int, danceability, energy, tempo, loudness, song_hotness)'
    c.execute(q)
    # commit and close
    conn.commit()
    c.close()
...
def fill_from_h5(conn, h5path, verbose=0):
    ...
    q += encode_string(str(track_id))
    title = get_title(h5)
    q += ', ' + encode_string(str(title))
    song_id = get_song_id(h5)
    q += ', ' + encode_string(str(song_id))
    release = get_release(h5)
    q += ', ' + encode_string(str(release))
    artist_id = get_artist_id(h5)
    q += ', ' + encode_string(str(artist_id))
    artist_mbid = get_artist_mbid(h5)
    q += ', ' + encode_string(str(artist_mbid))
    artist_name = get_artist_name(h5)
    q += ', ' + encode_string(str(artist_name))
    ...
    q += ", " + str(song_hotness) if not np.isnan(song_hotness) else ",-1"
...
def add_indices_to_db(conn, verbose=0):
    ...
    q = "CREATE INDEX idx_danceability ON songs ('danceability')"
    if verbose > 0:
        print(q)
    c.execute(q)
    
    q = "CREATE INDEX idx_energy ON songs ('energy')"
    if verbose > 0:
        print(q)
    c.execute(q)
    
    q = "CREATE INDEX idx_tempo ON songs ('tempo')"
    if verbose > 0:
        print(q)
    c.execute(q)
    
    q = "CREATE INDEX idx_loudness ON songs ('loudness')"
    if verbose > 0:
        print(q)
    c.execute(q)
    
    q = "CREATE INDEX idx_song_hotness ON songs ('song_hotness')"
    if verbose > 0:
        print(q)
    c.execute(q)
...
if __name__ == '__main__':
    ...
    maindir = '/home/ec2-user/msd/data/A/A'
    dbfile = '/home/ec2-user/Notebooks/track_data.db'
    ...
    for root, dirs, files in os.walk(maindir):
        ...
        for f in files:
            print("File " + str(cnt_files) + ": " + str(f), end="")
            print('\r', end='')
